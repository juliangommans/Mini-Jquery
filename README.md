...
It's important to realize that jQuery is not just one thing, it's a collection of things grouped under one lib. In fact, some parts of it could be used as standalone tools. For example, jQuery uses [Sizzle](https://github.com/jquery/sizzle) under the hood as its selector engine.
...
Create a module called SweetSelector that allows us to do the following (hint: you need to do it in [pure javascript](http://youmightnotneedjquery.com/) ):
...
```javascript
EventDispatcher.on('.klass', 'fakeClick', function() { console.log("awesome") });
EventDispatcher.trigger('.klass', 'fakeClick');
